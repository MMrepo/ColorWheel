//
//  BrightnessHalfCircleImageCreator.swift
//  ColorWheel-iOS
//
...
class BrightnessHalfCircleImageCreator: CIColorImageCreator {
    public var hue: CGFloat
    
    var imageFactory: CIColorImageFactory
    private var arguments: [Any] {
        let startColor = UIColor(hue: self.hue, saturation: 1.0, brightness: 0.0, alpha: 1.0)
        let endColor = UIColor(hue: self.hue, saturation: 1.0, brightness: 1.0, alpha: 1.0)
    ...
    init() throws {
        hue = 0
        try imageFactory = CIColorImageFactory(shaderName: "GradientHalfCircle", fromBundle: Bundle(for: BrightnessHalfCircleImageCreator.self))
    }
